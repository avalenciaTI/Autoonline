...
public class TestLogInBasic extends WebTestBase {
        ...
        @Test(priority = 25)
        @TmsData.Tc(tcId = 160927, tcName = "CP025_Various indivaswaidual case record Diverse", tcType = TcType.REGRESSION)
        ...
        //Passed
        @Test(priority = 29)
        @TmsData.Tc(tcId = 3936395, tcName = "CP029_General inquiry transfers", tcType = TcType.REGRESSION)
        public void tc29_generalInquiryTransfer() {
                AolWebUser master = this.users.getMasterUser();
                loginPage().logIn(master);
                assertions().assertThat(caseIndividualRegistration().generalInquiryTransfer())
                                .as("GENERAL INQUIRY TRANSFERS SUCCESS").isTrue();
        }

        @Test(priority = 33)
        @TmsData.Tc(tcId = 3936399, tcName = "CP033_Advanced vehicle consultation", tcType = TcType.REGRESSION)
        public void tc33_advancedVehicleConsultation() {
                AolWebUser master = this.users.getMasterUser();
                loginPage().logIn(master);
                assertions().assertThat(caseIndividualRegistration().advancedConsultationVehicles(vin2))
                                .as("ADVANCED VEHICLE CONSULTATION SUCCESS").isTrue();
        }

        @Test(priority = 32)
        @TmsData.Tc(tcId = 3936398, tcName = "CP032_Advanced consultation transfers", tcType = TcType.REGRESSION)
        public void tc32_advancedConsultationTransfers() {
                AolWebUser master = this.users.getMasterUser();
                loginPage().logIn(master);
                assertions().assertThat(caseIndividualRegistration().advancedConsultationTransfers())
                                .as("ADVANCED CONSULTATION TRANSFERS SUCCESS").isTrue();
        }
